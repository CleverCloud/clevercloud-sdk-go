// Code generated by generate-services. DO NOT EDIT.

package kubernetes

import (
	"context"
	client "go.clever-cloud.dev/client"
	utils "go.clever-cloud.dev/sdk/internal/utils"
	models "go.clever-cloud.dev/sdk/models"
	attribute "go.opentelemetry.io/otel/attribute"
	trace "go.opentelemetry.io/otel/trace"
)

/*
Deletenode

Parameters:
  - ctx: context for the request
  - client: the Clever Cloud client
  - tracer: OpenTelemetry tracer for observability
  - ownerId:
  - clusterId: A Kubernetes cluster identifier
  - nodeId: A standalone node identifier

# Returns the operation result or an error

Example:

	response := kubernetes.Deletenode(ctx, client, tracer, ownerId, clusterId, nodeId)
	if response.HasError() {
		// Handle error
	}
	result := response.Payload()

x-service: kubernetes
operationId: deleteNode
*/
func Deletenode(ctx context.Context, c *client.Client, tracer trace.Tracer, ownerId string, clusterId string, nodeId string) client.Response[models.StandaloneNodeView] {
	ctx, span := tracer.Start(ctx, "deleteNode", trace.WithAttributes(attribute.String("ownerId", ownerId), attribute.String("clusterId", clusterId), attribute.String("nodeId", nodeId)))
	defer span.End()

	path := utils.Path("/v4/kubernetes/organisations/%s/clusters/%s/nodes/%s", ownerId, clusterId, nodeId)

	// Make API call
	response := client.Delete[models.StandaloneNodeView](ctx, c, path)

	if response.HasError() {
		span.RecordError(response.Error())
	}

	return response
}
