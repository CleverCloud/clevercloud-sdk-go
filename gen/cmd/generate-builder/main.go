...
func extractOperations(spec *openapi31.Spec) []BuilderOperation {
	...
	for _, path := range paths {
		...
		// Process each HTTP method in sorted order
...
func generateBuilderCode(root *BuilderNode, config *Config) error {
	...
	// Collect unique service packages from all versions
	packages := make(map[string]bool)

	// Generate builders for each API version that exists
	versions := []struct {
		path   string
		prefix string
	}{
	...
	for _, version := range versions {
		if versionNode, ok := root.Children[version.path]; ok {
			collectPackages(versionNode, packages)
			// Generate builders recursively for this version
			generateBuilderNodes(f, versionNode, version.prefix, []FieldInfo{{Name: "sdk", Type: Op("*").Id("sdkImpl")}}, packages)
		}
	}
