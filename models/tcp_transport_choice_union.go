...
// TcpTransportChoice `TcpTransport.Transport` (:453-457).
// Tagged union - can hold one of: map[string]any, map[string]any, map[string]any
type TcpTransportChoice struct {
	raw json.RawMessage
}
...
func (u TcpTransportChoice) Type() string {
	t, _ := peekType(u.raw)
	return t
}

// MarshalJSON returns the raw JSON payload of the held value, or null if empty.
func (u TcpTransportChoice) MarshalJSON() ([]byte, error) {
	if u.raw == nil {
...
func (u *TcpTransportChoice) UnmarshalJSON(data []byte) error {
