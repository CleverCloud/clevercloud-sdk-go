...
// UdpTransportChoice `UdpTransport.Transport` (:425-429).
// Tagged union - can hold one of: map[string]any, map[string]any, map[string]any
type UdpTransportChoice struct {
	raw json.RawMessage
}
...
func (u UdpTransportChoice) Type() string {
	t, _ := peekType(u.raw)
	return t
}

// MarshalJSON returns the raw JSON payload of the held value, or null if empty.
func (u UdpTransportChoice) MarshalJSON() ([]byte, error) {
	if u.raw == nil {
...
func (u *UdpTransportChoice) UnmarshalJSON(data []byte) error {
